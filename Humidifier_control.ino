...
// EncButton<EB_TICK, encA, encB, encKEY> enc;        //создание объекта енкодера enc и инициализация пинов с кнопкой для чипа Arduino NANO, EncButton v2.0
...
bool humSetChange = 0;   // флаг изменения заданного значения влажности
bool humDeltaChange = 0; // флаг изменения отклонения значения влажности
bool Screen = 0;         // 0 на экран выводим заданную влажность, 1 отклонение от заданного
bool screenChange = 0;   // флаг изменения экрана
...
void setup() {
 ...
 // attachInterrupt(0, encChange, CHANGE);  // прерывание на пин D2 с крутилки енкодера. Для EncButton v2.0, v3.0
 // attachInterrupt(1, encChange, CHANGE);  // прерывание на пин D3 с крутилки енкодера. Для EncButton v2.0, v3.0
 // enc.setEncISR(true);                    // для корректной работы EncButton v3.0
}
...
// Процедура обработчик прерываний с енкодера. Используется для EncButton v2.0 и v3.x 
...
// void encChange() {  
//      enc.tickISR();                  // для EncButton v3.x
//      enc.tick();                     // для EncButton v2.0
//      if(enc.turn()){                 // если енкодер повернут без нажатия. EncButton v2.0
//       Screen = !Screen;              // обращаем выводимый экран. EncButton v2.0
//       screenChange = 1;              // поднимаем флаг изменения выводимого экрана. EncButton v2.0 
//      }                               // end if. EncButton v2.0
//      if (Screen && enc.rightH()) {   // если нажатый поворот направо на первом экране. EncButton v2.0
//        humDelta++;                   // увеличиваем значение отклонения влажности. EncButton v2.0
//        humDeltaChange = 1;           // поднимаем флаг изменения отклонения влажности. EncButton v2.0
//      }                               // end if. EncButton v2.0
//      if (Screen && enc.leftH()) {    // если нажатый поворот налево на первом экране. EncButton v2.0
//        humDelta--;                   // уменьшаем значение отклонения влажности. EncButton v2.0
//        humDeltaChange = 1;           // поднимаем флаг изменения отклонения влажности. EncButton v2.0
//      }                               // end if . EncButton v2.0      
//      if (!Screen && enc.rightH()) {  // если нажатый поворот направо на нулевом экране. EncButton v2.0
//        humSet++;                     // увеличиваем значение заданной влажности. EncButton v2.0
//        humSetChange = 1;             // поднимаем флаг изменения заданного значения влажности. EncButton v2.0
//      }                               // end if. EncButton v2.0
//      if (!Screen && enc.leftH()) {   // если нажатый поворот налево на нулевом экране. EncButton v2.0
//        humSet--;                     // уменьшаем значение заданной влажности. EncButton v2.0
//        humSetChange = 1;             // поднимаем флаг изменения заданного значения влажности. EncButton v2.0
//      }                               // end if . EncButton v2.0            
// }  // end encChange
