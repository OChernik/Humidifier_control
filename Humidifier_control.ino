...
EncButton enc(encA, encB, encKEY); //создание объекта енкодера enc и инициализация пинов с кнопкой <A, B, KEY> для чипа Arduino NANO, EncButton v3.x
// EncButton<EB_TICK, encA, encB, encKEY> enc;       //создание объекта енкодера enc и инициализация пинов с кнопкой <A, B, KEY> для чипа Arduino NANO, EncButton v2.0
// EncButton<EB_TICK, encA, encB, encKEY> enc(INPUT);       //создание объекта енкодера enc и инициализация пинов с кнопкой <A, B, KEY> INPUT - для чипа LGT8F328P, EncButton v2.0
...
int8_t humSet = 50;      // начальное значение заданной влажности, изменяется в функции по прерыванию
int8_t humDelta = 3;     // начальное значение отклонения от заданной влажности, изменяется в функции по прерыванию
bool humSetChange = 0;   // флаг изменения заданного значения влажности, изменяется в функции по прерыванию
bool humDeltaChange = 0; // флаг изменения отклонения значения влажности, изменяется в функции по прерыванию
bool Screen = 0;         // 0 на экран выводим заданную влажность, 1 отклонение от заданного, изменяется в функции по прерыванию
bool screenChange = 0;   // флаг изменения экрана, изменяется в функции по прерыванию
// volatile int8_t humSet = 50;      // начальное значение заданной влажности, изменяется в функции по прерыванию. При использовании EncButton v.2.0
// volatile int8_t humDelta = 3;     // начальное значение отклонения от заданной влажности, изменяется в функции по прерыванию. При использовании EncButton v.2.0
// volatile bool humSetChange = 0;   // флаг изменения заданного значения влажности, изменяется в функции по прерыванию. При использовании EncButton v.2.0
// volatile bool humDeltaChange = 0; // флаг изменения отклонения значения влажности, изменяется в функции по прерыванию. При использовании EncButton v.2.0
// volatile bool Screen = 0;         // 0 на экран выводим заданную влажность, 1 отклонение от заданного, изменяется в функции по прерыванию. При использовании EncButton v.2.0
// volatile bool screenChange = 0;   // флаг изменения экрана, изменяется в функции по прерыванию. При использовании EncButton v.2.0
...
void setup() {
 ...
 // описываем прерывания, по которым будут обрабатываться показания енкодера, процедуру обработчик прерываний  
 // attachInterrupt(0, encChange, CHANGE);  // прерывание на пин D2 с крутилки енкодера. Для EncButton v2.0
 // attachInterrupt(1, encChange, CHANGE);  // прерывание на пин D3 с крутилки енкодера. Для EncButton v2.0
}

void loop() {  
  if enc.tick() {                     // функция обработки сигнала (тикер) с энкодера. Для EncButton v3.x
...
// Процедура обработчик прерываний с енкодера. Используется для EncButton v2.0. 
