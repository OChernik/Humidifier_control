...
void setup() {
  ...
  sensor.begin(0x40);          // инициализируем датчик HDC1080 с настройками по умолчанию: нагреватель выключен, 14 бит разрешение
  // sensor.begin();           // инициализируем датчик BME280/BMP280 с настройками по умолчанию
  // sensor.begin(0x44)) {     // инициализируем датчик SHT31. Set to 0x45 for alternate i2c addr
...
void loop() {  
  if (enc.tick()) {                                   // функция обработки сигнала (тикер) с энкодера 
    ...
    if (humDelta < 0) humDelta = 0;                   // проверка допустимости диапазона значений humDelta
    if (humDelta > 5) humDelta = 5;                   // проверка допустимости диапазона значений humDelta
    if (humSet < 5) humSet = 5;                       // проверка допустимости диапазона значений humSet
    if (humSet > 95) humSet = 95;                     // проверка допустимости диапазона значений humSet
  ...
  if (millis() - tmr >= period){               // если пришло время считать данные с датчика
      ...
      temperature = sensor.readTemperature();  // сохраняем текущее значение температуры
      humGet = sensor.readHumidity();          // сохраняем текущее значение влажности
      showScreen();                            // выводим переменные на дисплей     
  }  // end if 
