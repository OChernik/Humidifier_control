...
void loop() {  
  if (enc.tick()) {                                   // функция обработки сигнала (тикер) с энкодера 
    ...
    if (!screen && enc.rightH() && enc.fast()) humSet = humSet + 5; // если нажатый быстрый поворот направо на нулевом экране увеличиваем заданную влажность на 5     
    if (!screen && enc.leftH() && enc.fast()) humSet = humSet - 5; // если нажатый быстрый поворот налево на нулевом экране уменьшаем заданную влажность на 5
